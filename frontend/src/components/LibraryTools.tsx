...
import { Modal, Spinner, issueLabel, formatBytes } from "./common";
 ...
 *  across, e.g., a volumes folder and a issues folder. */
 ...
 *  the right entry, and re-sync the issue list from the corrected links. */
export function SourcesModal({
  ...
  const resync = useMutation({
    mutationFn: () => api.post<{ issues: number; matched_issues: number }>(
      `/series/${seriesId}/resync`,
    ),
    onSuccess: (r) => {
      setResyncMsg(`Rebuilt: ${r.issues} issues, ${r.matched_issues} adopted from disk.`);
      invalidate();
    },
  });
  ...
  return (
    <Modal title="Edit sources" onClose={onClose}>
      <p className="section-hint">
        ...
        right entry. After fixing links, re-sync to rebuild the issue list.
      </p>
      ...
      <div style={{ marginTop: 16, display: "flex", gap: 10, alignItems: "center" }}>
        <button className="btn danger" disabled={resync.isPending} onClick={() => resync.mutate()}>
          {resync.isPending ? "Re-syncing…" : "Re-sync issues from links"}
        </button>
      ...
      {isLoading ? (
        ...
        <>
          <p className="section-hint">
            ...
            {result.repointed > 0 && `, re-pointed ${result.repointed} issue(s)`}
...
/** Preview + apply renames into pullarr's naming convention (Sonarr-style). */
 ...
 *  mapped to a issue (or a issue range for whole-volume archives). */
export function FilesModal({
  ...
  const map = useMutation({
    mutationFn: (args: { file_path: string; issue_id: number }) =>
  ...
  // a volume archive is matched by covering issues, not by being a
  // single issue — only files covering nothing need manual mapping
  const unmatched =
    data?.filter((f) => f.matched_issue_id == null && f.covered_count === 0) ?? [];
  const matched =
    data?.filter((f) => f.matched_issue_id != null || f.covered_count > 0) ?? [];
                    ...
                    <tr key={f.path}>
                      ...
                      <td style={{ width: 180 }}>
                        <select
                          ...
                          onChange={(e) =>
                            e.target.value &&
                            map.mutate({ file_path: f.path, issue_id: Number(e.target.value) })
                          }
                        >
