...
from pullarr import notifications


@respx.mock
async def test_send_webhook_posts_secret_header():
    route = respx.post("http://panel.test/api/v1/webhooks/pullarr").mock(
        return_value=Response(204)
    )
    ok = await notifications.send_webhook(
        "http://panel.test/api/v1/webhooks/pullarr", "s3cret",
        {"app": "pullarr", "event": "import", "series_id": 7},
    )
    assert ok is True
...
async def test_notify_import_fire_and_forget():
    ...
    notifications.notify_import(values, 42, "Issue 12")
