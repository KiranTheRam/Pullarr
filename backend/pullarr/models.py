...
class Series(Base):
    """A comic series — one ComicVine "volume" (e.g. Batman (2016))."""
    ...
    year: Mapped[int | None] = mapped_column(Integer, nullable=True)  # start year
    ...
    issues: Mapped[list[Issue]] = relationship(
        back_populates="series", cascade="all, delete-orphan", order_by="Issue.number"
    )
...
class SeriesFolder(Base):
    ...
    folder (Series.folder_name). Lets one series span, e.g., an issues folder
    and a separate TPB folder."""
...
class SeriesSourceLink(Base):
    """Maps a series to its identity on a content source. For GetComics the
    external_id is the search term whose posts belong to this series (usually
    the series title; overridable when the site titles it differently)."""
...
class Download(Base):
    ...
    payload: Mapped[str] = mapped_column(Text, default="")  # source-specific: post URL / magnet
